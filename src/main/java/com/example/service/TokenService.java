...
public class TokenService {
    ...
    public TokenResponse login(String username, String password, String deviceId) {
        ...
        Instant accessExp = now.plusMillis(jwtTokenProvider.getAccessTokenValidityMs());
        Instant refreshExp = now.plusMillis(jwtTokenProvider.getRefreshTokenValidityMs());
        ...
        session.setAccessTokenExpiry(accessExp);
        session.setRefreshTokenExpiry(refreshExp);
    ...
    public TokenResponse refresh(String refreshToken) {
        ...
        Instant now = Instant.now();
        Instant newAccessExp = now.plusMillis(jwtTokenProvider.getAccessTokenValidityMs());
        Instant newRefreshExp = now.plusMillis(jwtTokenProvider.getRefreshTokenValidityMs());

        session.setStatus(SessionStatus.USED);
        userSessionRepository.save(session);

        UserSession newSession = new UserSession();
        newSession.setUserEmail(username);
        ...
        newSession.setAccessTokenExpiry(newAccessExp);
        newSession.setRefreshTokenExpiry(newRefreshExp);
